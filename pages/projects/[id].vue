...
const { data: project, pending, error } = await useFetch(`/api/projects/${encodeURIComponent(projectId)}`);
...
<template>
  <main class="flex flex-col items-center justify-center p-10 text-white">
    ...
    <div v-else-if="project">
      ...
      <img :src="project.img" :alt="project.name" class="rounded-lg shadow-lg max-w-lg mb-6" />
      <p class="mb-4">{{ project.description }}</p>
      <a :href="project.url" class="text-blue-400 hover:underline">Voir le projet sur GitHub</a>
